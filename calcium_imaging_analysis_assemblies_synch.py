...
############################################analyze spike frequency################################################
    def deconv_freq(self):
        ...
        #output = spike_freq_time = mean spike density
        print('the average spike frequency is: ' + str(sum(spike_freq_time)/len(spike_freq_time)))
...
###########################calculate % that each cell contributes to synch events ##################################################################   
    ...
    #output = reordered_cluster_amp
        ...
        plt.figure()
        subplot(2,1,1)
        pyplot.hist(clusters2[0])
        axvline(x=sil_score_bycluster[0] , color='red')
        plt.title('Cluster 1 cumulative probability after 1000 shuffles') 

        subplot(2,1,2)
        pyplot.hist(clusters2[1])
        axvline(x=sil_score_bycluster[1] , color='red')
...
#
        ...
        unique, counts = numpy.unique(just, return_counts=True)
        ...
        ax.set_ylabel('Contribution (95%) of cluster 1 to synchronized calcium events')
        ax.set_xlabel('Frames')
        #ax.set_title('Cell #21 and #41') 

        #
        ax = plt.subplot(2,2,2) 
        ax.set_ylabel('Contribution (95%) of cluster 2 to synchronized calcium events')
        ax.set_xlabel('Frames')
    ...
    def summary_plots(self, t_sne_perplex, plot_1,plot_2,plot_3,plot_4):
        ...
        if plot_1 == 'true':
            ...
            subplots_adjust(left=0.09, bottom=0.02, right=0.7, top=0.80, wspace=0.4, hspace=0.5)
            plt.show()
        ...
        if plot_3 == 'true':
            ...
            #plot hist frame 1          
            plt.subplot(3,2,2)
            pyplot.hist(hist_array[0][175], alpha=0.5, label='hist_array', color='gray')
            plt.ylabel('Count')
            plt.xlabel('Spikes')
            axvline(x=sum_o_spikes_array[0][175] , color='red')
            plt.title('Frame #175') 
            pyplot.legend(loc='upper right')
          
            #plot hist frame 2
            plt.subplot(3,2,3)
            pyplot.hist(hist_array[0][20], alpha=0.5, label='hist_array', color='gray')
            plt.ylabel('Count')
            plt.xlabel('Spikes')
            axvline(x=sum_o_spikes_array[0][20], color='red')
            plt.title('Frame #20') 
            pyplot.legend(loc='upper right')
            ...
            plt.xlabel('Frames')
            plt.title('All Cells')
            axhline(y=0.99, color='red', linestyle=':')     
            ...
            subplots_adjust(left=0.09, bottom=0.1, right=0.9, top=0.9, wspace=0.5, hspace=0.5)
            plt.show()
